...
// Backend: replace with current user wallet from Firestore
const WALLET = {
  available: 248,
  pending: 150,
  totalWithdrawn: 250,
...
export default function WithdrawBalanceCard() {
  const isEligible = WALLET?.available >= 100;
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div className="md:col-span-1 relative overflow-hidden rounded-2xl p-6"
        ...
        <div className="mt-3 flex items-center gap-2">
          {isEligible ? (
            ...
            <span className="text-xs text-yellow-400">Need ₹{100 - WALLET?.available} more to withdraw</span>
          )}
        </div>
      </div>
      {/* Pending */}
      <div className="glass-card p-5 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-2xl font-bold text-yellow-400 font-mono tabular-nums">₹{WALLET?.pending}</p>
      ...
      <div className="glass-card p-5 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-2xl font-bold text-green-400 font-mono tabular-nums">₹{WALLET?.totalWithdrawn}</p>
          <p className="text-xs text-white/30">Across 2 withdrawals</p>
        </div>
      </div>
    </div>
  );
